...
def plot_template(temp, clrs, ctr, geom, chans=None, time_scale = 3., scale = 10., figsize=(12,9), savepath=None):
    max_chan = np.arange(temp.shape[1])

    fig = plt.figure(figsize=figsize)
    plt.plot(geom[max_chan,0]+np.arange(-temp.shape[0],0)[:,np.newaxis]/time_scale, 
                         geom[max_chan,1] + temp[:,max_chan]*scale,color=clrs[ctr], alpha=.7)

    
    for k in range(temp.shape[1]):
        plt.text(geom[k,0]+1, geom[k,1], str(k), fontsize=15)

    x = np.arange(-temp.shape[0],0)/time_scale
    y = -np.ones(x.shape)*scale
    y2 = np.ones(x.shape)*scale
    for k in range(temp.shape[1]):
        plt.fill_between(x+geom[k,0], y2 + geom[k,1], y + geom[k,1], color='grey', alpha=0.1)

    if chans is not None:
        plt.scatter(geom[chans,0], geom[chans,1],c='blue')
...
def plot_raw_spikes_in_rows(waveforms, assignments, spacing=1, width=1, vscale=1, subplot_adj=0.9,
                            colors=default_colors, figtitle="", figdir="./", fname_postfix="", show=True):
    """
    waveforms: [n_samples, n_timesteps, n_channels]
    """
    ...
    waveforms_plot = waveforms * vscale - np.reshape(np.arange(n_samples), (-1, 1, 1)) * spacing * 8

    fig_height = 1
    fig, axes = plt.subplots(1, n_chs, figsize=(width * n_chs, 2 + (n_samples - 1) * spacing / 4), sharey=True)
    fontsize = 15
    # if ylim is not None:
    #     plt.ylim(*ylim)
    # else:
    ...
    #axes[0].legend()
    fig.suptitle(figtitle, fontsize=fontsize+13, y=1 - (1-0.99) * 100 / n_samples)
    plt.tight_layout()
    ...
    if show:
        ...
        save_path = os.path.join(figdir, "sample_{}_{}.png".format(n_samples, fname_postfix))
...
def plot_raw_spikes_overlay(waveforms, assignments, geom, channels, colors, sort_by_count=True,
    ...
    Args:
        ...
        assignments: 
        geom: [n_channels, 2]
        channels: [n_channels] ordered list of channel_id correspond to the channel axis of waveforms
    """
    ...
    for i, cl in enumerate(clusters):
        ...
        time_axis = np.tile(np.arange(n_timesteps), len(wave_subset)).reshape((-1, n_timesteps))[:,:,np.newaxis]
        ...
        if not vertical:
            axes[i][0].text(geom_use[:, 0].max(), geom_use[:, 1].max(), "n = {}".format(count), fontsize=20)
        else:
        ...
        axes[i][1].plot(plot_x, plot_y, color=colors[i], alpha=alpha_single, linewidth=3) 

        geom_y_range = max(geom_use[:, 1].max() - center[1], center[1] - geom_use[:, 1].min()) * 2
        geom_x_range = max(geom_use[:, 0].max() - center[0], center[0] - geom_use[:, 0].min()) * 2
        if not vertical:
        ...
        for k in range(n_channels):
            for col in [0,1]:
                if not vertical:
                    axes[i][col].text(geom_use[k, 0] + n_timesteps / 2 / time_scale + 1, geom_use[k, 1] + 5, str(channels[k]), fontsize=15)
                axes[i][col].fill_between(fill_x + geom_use[k,0], 
                        wave_subset_std[:,k] * scale + plot_y[:,k], 
                        -wave_subset_std[:,k] * scale + plot_y[:,k], 
                        color='grey', alpha=0.1)
    if vertical:
        for k in range(n_channels):
            axes[0][0].text(geom_use[k, 0] - n_timesteps / 2 / time_scale - 6, geom_use[k, 1] + 5, str(channels[k]), fontsize=15)
...
def plot_templates_separate(templates, geom, channels, colors, 
    ...
    for i in np.arange(n_templates):
        ...
        axes[i].plot(plot_x, plot_y, color=colors[i], alpha=alpha_single, linewidth=3) 
        ...
        for k in range(n_channels):
            if not vertical:
                axes[i].text(geom_use[k, 0] + n_timesteps / 2 / time_scale + 1, geom_use[k, 1] + 5, str(channels[k]), fontsize=15)
            # axes[i].fill_between(fill_x + geom_use[k,0], fill_y2 + geom_use[k,1], fill_y1 + geom_use[k,1], color='grey', alpha=0.1)
    
    if vertical:
        for k in range(n_channels):
            axes[0].text(geom_use[k, 0] - n_timesteps / 2 / time_scale - 6, geom_use[k, 1] + 5, str(channels[k]), fontsize=15)
        fig.suptitle(figtitle, fontsize=titlesize)
    else:
    ...
    if show:
        ...
        save_path = os.path.join(figdir, "{}_templates_{}.png".format(fname_postfix, n_templates))
        plt.savefig(save_path)
        plt.close()
        return save_path
...
def plot_spike_clusters_and_gt_in_rows(clusters, nll, data_arr, gt_labels, topn=2, figdir="./", fname_postfix="",
    ...
    combine_imgs(fig_paths, fpath.replace("pred", "pred"), downsample=downsample)
    for f in fig_paths:
        if os.path.exists(f):
            os.remove(f)
...
def plot_raw_and_encoded_spikes_tsne(clusters, nll, data_arr, data_encoded, default_colors, topn=2, 
    ...
    if extra_clusters is not None and extra_name is not None:
        n_plots += 1
        plot_clusters = np.concatenate([topn_clusters[:topn], [extra_clusters]])
    ...
    fig, axes = plt.subplots(n_plots, 2, figsize=(size_single[0] * 2, size_single[1] * n_plots))
    if n_plots == 1:
        axes = [axes]
    tsne = TSNE(n_jobs=4, n_components=2, perplexity=tsne_params['perplexity'], random_state=tsne_params['seed'])
    ...
    for i in range(n_plots):
        ...
        color_mapping = {cluster_ids[i]:(default_colors[i] if cluster_ids[i] in large_clusters else 'grey') 
                            for i in range(len(cluster_ids))}

        fontsize = 12
        for k in cluster_ids:
            mask = (clusters_use==k)
            axes[i][0].scatter(tsne_raw[mask,0], tsne_raw[mask,1], color=color_mapping[k], s=pt_size)
            axes[i][1].scatter(tsne_encoded[mask,0], tsne_encoded[mask,1], color=color_mapping[k], s=pt_size)

        axes[i][0].set_title('t-SNE of raw spikes ({})'.format(plot_clusters_names[i]), fontsize=fontsize)
        axes[i][1].set_title('t-SNE of NCP-encoded spikes ({})'.format(plot_clusters_names[i]), fontsize=fontsize)
...
def plot_samples_tsne(dpmm, data_generator, N= 50, seed = None, save_name=None, s=26):
    ...
    for i in range(5):  # top 5 by nll
        ...
        K=len(set(cs))
        prob = np.exp(-snll)
        ax[i+2].set_title(str(K) + ' Clusters    Prob: '+ '{0:.2f}'.format(prob), fontsize=fontsize)
        for axis in ['top','bottom','left','right']:
